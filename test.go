...
//	})
func AssertCLI(t testing.TB, c *Core, tc CLITest) {
	t.Helper()
	...
	if tc.WantOK != r.OK {
		t.Errorf("AssertCLI %q want.OK=%v got.OK=%v err=%v", label, tc.WantOK, r.OK, r.Error())
		return
	}
	if !r.OK {
		return
	...
	if tc.Output != "" && out != tc.Output {
		t.Errorf("AssertCLI %q stdout-want=%q got=%q", label, tc.Output, out)
	}
	if tc.Contains != "" && !Contains(out, tc.Contains) {
		t.Errorf("AssertCLI %q stdout-contains=%q got=%q", label, tc.Contains, out)
	}
}
